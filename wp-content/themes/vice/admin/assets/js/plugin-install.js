jQuery(document).ready(function($) {
    'use strict';
    var this_obj = vice_companion_install;
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'vice_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.webriti-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'vice_hide_customizer_notice';
            sdata.vice_plugin_slug = slug;
        }else{
            sdata.action = 'vice_hide_customizer_companion_notice';
        }
